...
binary = str(
    Path(
        sys.argv[1] if len(sys.argv) > 1 else root / "core/target/debug/memnest"
    ).resolve()
)
...
with tempfile.TemporaryDirectory(prefix="memnest-bounded-") as data:
    ...
    def request(path, body=None) -> tuple[int, Any]:
        req = urllib.request.Request(
            base + path,  # noqa: S310 - fixed loopback HTTP base
    ...
    with tempfile.TemporaryFile(mode="w+") as log:
        ...
        child = subprocess.Popen(
            [binary, "--data-dir", data, "--port", str(port)],
            stdout=log,
            stderr=log,
            env=env,
        )
        try:
            ...
            for id, project, session, source, cwd in fixtures:
                timestamp = (
                    "2026-01-01T00:00:"
                    + ("00" if id == "before" else "01" if id == "anchor" else "02")
                    + "+00:00"
                )
                metadata = {
                    "session_id": session,
            ...
            assert full["document"].startswith(
                "😀한é"
            ) and "supersecret123" not in json.dumps(full)
            ...
            assert (
                json.loads(
                    mcp("memory_get", {"id": "metadata-heavy", "max_chars": 1})[
                        "content"
                    ][0]["text"]
                )
                == heavy
            ...
            assert (
                page["document"] == full["document"][1:8] and page["next_offset"] == 8
            )
            ...
            assert all(
                c["has_more"] and c["next_offset"] == 0
                for c in page["before"] + page["after"]
            )
            _, expanded = request("/chunk/anchor?max_chars=30000&before=5&after=5")
            assert expanded["total_returned_chars"] == 3 * full["doc_len"]
            assert all(
                "supersecret123" not in c["document"]
                for c in expanded["before"] + expanded["after"]
            )
            ...
            got = mcp(
                "memory_get",
                {"id": "anchor", "offset": 1, "max_chars": 7, "before": 5, "after": 5},
            )
            ...
            for subject in [
                "apples orchard harvest",
                "database postgres connection",
                "satellite orbit telescope",
            ]:
                status, saved = request(
                    "/add",
                    {
                        "project": "budget",
                        "text": ("budgetprobe " + subject + " ") * 90,
                    },
                )
            ...
            assert [c["id"] for c in old["results"]] == [
                c["id"] for c in unchanged["results"]
            ]
            assert [len(c["document"]) for c in unchanged["results"]] == [600, 600, 600]
            result = mcp("memory_search", dict(search, max_chars=125))["content"][0][
                "text"
            ]
            ...
            print(
                "PASS real HTTP + MCP: Unicode pagination, get page/provenance caps, isolation, internal buckets, redaction, validation; all three 600-character search excerpts preserved."
            )
        except Exception:
            log.flush()
            log.seek(0)
