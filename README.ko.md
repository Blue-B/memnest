...
두 색인 모두 저장소에 실제 답이 있는지는 판단하지 못합니다. 답이 저장된 적 없는 질문에도 가장 가까운 기억을 돌려줄 수 있으므로, 검색 결과는 답이 존재한다는 증거가 아니라 확인해야 할 후보로 다루세요.
...
캐시는 약 465 MB, 최대 메모리는 약 1.1 GB로 줄지만 검색 품질은 기본 모델보다 낮습니다. 기존 벡터는 예전 차원으로 기록되어 있어서 데이터가 쌓인 뒤에 이 값을 바꾸면 색인을 전부 다시 만들어야 합니다.

Linux, WSL, Windows 서비스 설정과 백업, 복구, 보존 정책은 [`docs/operations.md`](docs/operations.md)에 있습니다.

현재 버전은 `0.1.0`입니다. 테스트로 확인하는 호환 범위는 저장 형식과 툴 이름 5개, HTTP 경로이며, 업그레이드는 이 셋을 지키는 데 가장 공을 들입니다. 응답 필드 이름과 환경 변수 기본값, 순위 가중치는 그 범위 밖이라 패치 릴리스에서도 바뀔 수 있습니다. 업그레이드 전에 `memory.db`와 `master.key`를 백업하세요.
