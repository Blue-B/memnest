...
<img src="docs/dashboard.png" alt="palimpsest 대시보드" width="820" />
...
palimpsest는 AI 에이전트를 위한 로컬 우선(local-first) 메모리 시스템입니다. 모든 데이터는
`~/.palimpsest/memory.db` 하나의 SQLite에 저장되며, 어떤 클라이언트를 쓰든 — Claude Code,
...
`~/.palimpsest/memory.db`를 공유하므로, 한 클라이언트에서 기록한 메모리를 다른 모든
클라이언트에서 검색할 수 있습니다.

| 클라이언트 | 연결 방법 |
| ------ | -------------- |
| Claude Desktop, Cursor, Cline, Continue, Zed, opencode | `palimpsest --mcp` 명령 등록 — [`pi-extension/INSTALL-CLIENTS.md`](./pi-extension/INSTALL-CLIENTS.md) 참고. |
| Claude Code, Codex CLI, Kilo Code, Windsurf 등 | MCP를 지원하는 다른 클라이언트도 동일한 `palimpsest --mcp` 등록 사용. |
| pi | 한 줄 설치: `pi install npm:pi-palimpsest` (도구 + AutoLog 추가). |
...
| 디렉터리 | 패키지 | 언어 | 역할 |
| --------- | ------- | ---- | ---- |
| [`core/`](./core) | `palimpsest` | Rust | 엔진: HTTP API + stdio MCP 서버, 하이브리드 검색, 비밀 보관소, 대시보드. 필수. |
| [`pi-extension/`](./pi-extension) | `pi-palimpsest` | TypeScript | [pi](https://github.com/badlogic/pi-mono)용 편의 브리지(도구 + AutoLog). 다른 클라이언트는 코어에 MCP로 직접 붙으므로 선택 사항입니다. |
| [`journal/`](./journal) | `palimpsest-journal` | TypeScript | 데이터베이스를 git 마크다운 저장소로 미러링하여, 에이전트가 학습한 내용을 diff·revert·검토할 수 있게 하는 감사 레이어. 선택. |
...
cd core
cargo build --release
./target/release/palimpsest          # 127.0.0.1:3111 에서 HTTP + 대시보드

# 2. 클라이언트 연결 (모든 MCP 클라이언트가 같은 명령 사용)
#    등록:  command "palimpsest", args ["--mcp"]
#    pi의 경우:
...
```bash
palimpsest                      # HTTP 서버 + 대시보드 (127.0.0.1:3111)
palimpsest --mcp                # stdio MCP 모드
palimpsest --doctor             # 환경 및 저장소 상태 점검
palimpsest --warmup-embedding   # 임베딩 모델 사전 로드
```

자주 쓰는 옵션: `--port`, `--host`, `--data-dir`, `--backup-dir`, `--restore-dir`,
`--import-jsonl`. 전체 목록은 `palimpsest --help`.
...
- 기본 바인딩은 `127.0.0.1`입니다. `PALIMPSEST_TOKEN`이 설정되지 않으면 외부 바인딩을 거부하며, 설정 시 `Bearer` 토큰이 필요합니다.
