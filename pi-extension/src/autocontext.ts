...
const N_RESULTS = Math.max(1, parseInt(env.MEMNEST_AUTOCONTEXT_N || "20", 10) || 20);
const TOP_INJECT = Math.max(1, parseInt(env.MEMNEST_AUTOCONTEXT_TOP || "2", 10) || 2);
const MAX_INJECTIONS = Math.max(1, parseInt(env.MEMNEST_AUTOCONTEXT_MAX_INJECTIONS || "6", 10) || 6);
const TOPIC_OVERLAP = Math.max(0, Math.min(1, Number(env.MEMNEST_AUTOCONTEXT_TOPIC_OVERLAP ?? "0.35")));
const MIN_SCORE = Number(env.MEMNEST_AUTOCONTEXT_MIN_SCORE ?? "0.12");
const MIN_LEN = Math.max(1, parseInt(env.MEMNEST_AUTOCONTEXT_MIN_LEN || "16", 10) || 16);
const TIMEOUT_MS = Math.max(200, parseInt(env.MEMNEST_AUTOCONTEXT_TIMEOUT_MS || "1500", 10) || 1500);
const DOC_CHARS = Math.max(80, parseInt(env.MEMNEST_AUTOCONTEXT_DOC_CHARS || "240", 10) || 240);
const EXCLUDE_PROJECTS = new Set(
  (env.MEMNEST_AUTOCONTEXT_EXCLUDE ?? "_superseded,default,root")
    .split(",")
    .map((s: string) => s.trim())
    .filter(Boolean),
);
...
function topicTokens(prompt: string): Set<string> {
  ...
  const stop = new Set(["the", "and", "for", "with", "this", "that", "그거", "이거", "좀", "해줘", "하면", "그리고", "근데"]);
...
function formatBlock(results: MemResult[], reason: string, options: { strong: boolean }): string | null {
  const kept = results
    .filter((r) => typeof r.document === "string" && r.document.trim().length > 0)
    .filter((r) => !(r.project && EXCLUDE_PROJECTS.has(r.project)))
    .filter((r) => (typeof r.score === "number" ? r.score : 1) >= MIN_SCORE)
    .slice(0, TOP_INJECT);
...
export function installAutocontext(pi: ExtensionAPI): void {
  ...
  if (!disabled) {
    pi.on("before_agent_start", async (event: unknown) => {
      const e = event && typeof event === "object" ? event as { prompt?: unknown } : {};
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      query: Type.Optional(Type.String({ description: "Optional: run a live retrieval and preview the block." })),
    }),
