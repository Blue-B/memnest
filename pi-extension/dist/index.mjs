...
function register(pi) {
  ...
  pi.registerCommand?.("memnest", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (!parsed) {
        const message2 = `Memnest returned an unreadable reply from ${URL}`;
  ...
  registerTool(
    ...
    "memory_remember",
    "Memory: remember",
    "Save something the next session should still know. Call it without being asked when the user corrects you, states a preference or a decision, or when you learn a config value, port, path, or a fix that took real effort to find. Skip whatever the next session can re-derive by reading the repo. Omitting project stores it in the current directory's workspace, which is right for anything specific to this codebase; pass project='playbook' instead when the lesson holds anywhere, because playbook is searched from every directory while a workspace is not. Set importance to preference for a correction or a stated preference, decision for a chosen approach, knowledge for a stable fact, log for routine detail. Pass supersedes=<id> when this replaces an existing memory instead of adding to it. Credentials, tokens and passwords go to secret_set, never here.",
    typebox_exports.Object({
      ...
      importance: typebox_exports.Optional(
        typebox_exports.Union(
          ...
          {
            default: "knowledge",
            description: "preference when the user corrects you or states how they want things done, decision for an approach chosen among alternatives, knowledge for a stable fact, log for routine detail. Pick deliberately; the default is only for when none of the others fit."
          }
        )
      ),
      memory_kind: typebox_exports.Optional(
        typebox_exports.Union(
          ...
          {
            default: "record",
            description: "fact for stable project or environment knowledge, rule for a preference or guardrail, procedure for a reusable workflow, record for a one-off outcome."
          }
        )
      ),
