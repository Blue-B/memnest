...
mod retrieval_eval {
    ...
    async fn retrieval_baseline_recall_and_mrr() {
        ...
        for (q, g) in &queries {
            let items = run_hybrid_search(system.clone(), q, "all", k, false, false, false, None).await;
    ...
    async fn top_ids_and_embeddings(
        ...
        let items = run_hybrid_search(system.clone(), query, "all", k, false, false, false, None).await;
        let sys = system.read().await;
        let db = sys.db.read().await;
        let mut ids = Vec::new();
    ...
    async fn mcp_search_matches_http_ranking() {
        ...
        let http = run_hybrid_search(system.clone(), query, "all", 5, false, false, false, None).await;
    ...
    async fn neighbors_finds_semantic_duplicates() {
        ...
        assert!(out.iter().all(|n| n.id != "d_hnsw"), "self should be excluded");
        for w in out.windows(2) {
            assert!(w[0].distance <= w[1].distance + 1e-6, "distances must be ascending");
        }
