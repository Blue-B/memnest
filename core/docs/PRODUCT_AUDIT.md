...
Make Memnest credible as a paid local product across Linux native, WSL, and Windows native installs, with a dashboard, search behavior, service lifecycle, release packaging, supportability, and security posture that can be validated before sale.
...
| Linux native install | `scripts/install-linux.sh`, `packaging/systemd/memnest.service`, `packaging/systemd/memnest-user.service`. |
...
| Windows native install without WSL | `scripts/install-windows.ps1`, `packaging/windows/memnest-service.xml`, WinSW wrapper handling. |
...
| Windows service data location | Windows native defaults to `%ProgramData%\Memnest` so service data is not tied to an interactive user's profile. |
...
| Remote bind safety | Non-localhost binds require `MEMNEST_TOKEN`; verified by `scripts/smoke-local.sh`. |
...
| First-use embedding model readiness should be visible before offline use | `memnest --warmup-embedding` warms the model cache; `memnest --doctor` warns when the cache is not warmed. |
...
- `target/release/memnest --help`: exposes `--warmup-embedding`.
- `target/release/memnest --doctor` against an empty temp data directory: warned that the model cache is not warmed.
...
- `scripts/preflight-linux.sh --user --bin target/release/memnest`: passed on the current WSL environment.
