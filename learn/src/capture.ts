...
/**
 * Decide which bucket a captured memory belongs in. buildInjection's
 * learned_rules slot searches ONLY `playbook`, so a durable cross-project lesson
 * (a correction or a user preference, both of which land on importance
 * preference|decision) has to be written there or it is never injected back.
 * Everything weaker stays project-local. Mirrors the pi-extension's
 * `resolveProject`, which applies the same rule to manual `memory_remember`.
 */
function routeProject(importance: Importance, project?: string): string {
  if (importance === "preference" || importance === "decision") return "playbook";
